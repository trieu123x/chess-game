...
> **Đã đạt.** 100 bàn (200 kết nối) chạy hết ván, 0 lỗi, move p95 ≈ 109 ms. Ngắt kết nối
> giữa ván rồi `RESUME`: 0 nước đi bị mất, recovery p50 ≈ 15 ms. Số liệu ở `statics/results/e1`
> và `statics/results/e5`.
...
> Toàn bộ E1–E8 chạy bằng **một lệnh**: `node tools/run-experiments.mjs --repeat 3`.
> Script tự bật/tắt rules service + server với đúng cấu hình của từng kịch bản, chạy bot,
> gộp CSV thô thành `summary.csv` và vẽ chart. Đây là điều kiện để §14 của README nói được
> rằng kết quả tái lập được — người chấm chạy lại một lệnh, không phải làm lại bằng tay.
...
Các ca chạy dài (M3, T18, T19, T20) tách sang một công cụ riêng vì chúng mất hàng chục phút,
không nên nằm chung với bộ E1–E8:

```bash
node tools/run-endurance.mjs --only m3 --minutes 10    # M3: 50 bàn chạy liên tục
node tools/run-endurance.mjs --only t18                # tắt PostgreSQL giữa chừng
node tools/run-endurance.mjs --only t19 --minutes 60   # kiểm rò bộ nhớ
node tools/run-endurance.mjs --only t20                # ngắt gateway
```

T18 cần chạy từ dòng lệnh **có quyền Administrator** (vì phải `net stop` dịch vụ PostgreSQL).

## 8.1. Chạy bộ test này bằng gì

Mỗi ca ở trên đều có chỗ chạy cụ thể, không phải làm tay:

| ID | Chạy bằng |
| --- | --- |
| T01, T03, T04, T15 | `node source/bot/cheat.js` — nhóm "frame hỏng ở mức byte", "MOVE khi chưa LOGIN", "flood", "MOVE cho ván của người khác" |
| T02 | `mvn test` → `FrameCodecTest` (cắt frame làm 3 lần ghi, ghi 5 frame một lần) |
| T05, T06, T07 | `node source/bot/bot.js --drop-at N --resume-after S` và kịch bản E5 trong `tools/run-experiments.mjs` |
| T08 | `--drop-at` với `lastPly` lớn hơn số nước server có — server luôn gửi full snapshot trước |
| T09, T11 | `cheat.js` (đăng nhập trùng, tự ghép với chính mình) |
| T12, T13, T14 | `mvn test` → `EmbeddedRulesTest` và `node source/rules-service/test.js` |
| T16 | `clock.compensationCapMs` — E4 trong `run-experiments.mjs` |
| T17 | E7 kịch bản "giết rules service giữa chừng" |
| T18 | Tắt PostgreSQL 60 s, xem `pending-moves.log` và `[stats] dbQueue=` |
| T19 | Chạy dài với `[stats] heapMB=` — RAM không được tăng tuyến tính |
| T20 | Tắt gateway khi `spectate.js` đang chạy (E8) |

Kết quả chạy thật ghi vào phụ lục báo cáo §8.19 và bảng §12 của README.
