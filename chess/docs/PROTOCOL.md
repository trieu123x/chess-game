# Protocol Specification – CGP v1.2 & RVP v1.1
...
| 0x10 | LOBBY_REQ | rỗng | **v1.2**: xin danh sách ván đang chạy + bảng xếp hạng. Không cần đăng nhập |
...
| 0x87 | PEER_STATUS | JSON `{state:"disconnected"/"reconnected"/"searching", graceMs, waitedMs}` | **v1.2** thêm `searching`: chờ ghép quá lâu thì báo thay vì im lặng (X23) |
| 0x88 | DRAW_OFFERED | rỗng | |
| 0x89 | SPECTATOR_COUNT | binary 2 byte `u16` | |
| 0x8A | HISTORY_RESULT | JSON `{pgn, moves[]}` | |
| 0x8B | LOBBY_RESULT | JSON `{games[], leaderboard[]}` | **v1.2**: trả lời `LOBBY_REQ` |
...
**Server gửi `HEARTBEAT` ngay sau `LOGIN_OK` và sau `RESUME`**, không đợi tới nhịp 5 s đầu tiên.
Không có mẫu RTT thì không bù được, và những nước đi đầu ván — đúng lúc chưa ai kịp nghĩ gì —
sẽ bị trừ oan trọn một vòng khứ hồi. Đo được ở E4: bỏ mẫu đầu này, drift p95 ở delay 150 ms
là **316 ms/nước**; có nó, còn **178 ms/nước**.

**Vì sao chỉ bù `rtt/2` chứ không bù cả `rtt`.** Khoảng thời gian bị tính cho người chơi kéo
dài trọn một vòng khứ hồi: server → client (gửi state) + thời gian nghĩ + client → server
(gửi nước đi). Bù đủ `rtt` sẽ xoá sạch thiệt hại, nhưng cũng biến đường truyền xấu thành
**lợi thế**: chỉ cần kết nối tệ là được cộng giờ. Bù một nửa là chọn có chủ đích — chấp nhận
còn lại một nửa thiệt hại để không ai có động cơ làm xấu kết nối của mình. Hệ quả đo được ở
E4 là drift giảm đúng `rtt/2` chứ không về 0, và đó là kết quả **đúng như thiết kế**, không
phải lỗi.
...
Client A        Server                       Client B
   ...
   │◄─── MATCH_FOUND + GAME_SNAPSHOT + MOVE_APPLIED(13..n) ──│
                  │──── PEER_STATUS(reconnected) ───────────────►│
```

Ba điểm của phần này, đã hiện thực và đo ở E5:

* **Snapshot đi trước, replay đi sau.** Client khai `lastPly` sai kiểu gì — kể cả
  `lastPly = 9999` (X14) — thì nó vẫn nhận được trạng thái đúng, vì snapshot mang
  đủ FEN, đồng hồ và danh sách nước đi. `lastPly` chỉ là *gợi ý* để server biết
  cần phát lại từ đâu, server không bao giờ tin nó.
* **Replay phát lại đúng byte đã gửi lần đầu**, không dựng lại từ FEN. Nhờ vậy
  chuỗi sự kiện mà client thấy không khác gì khi chưa từng mất kết nối.
* **`LOGIN` cũng nối lại được.** Nếu phiên cũ hết hạn và người chơi đăng nhập lại
  bằng mật khẩu, server vẫn gắn họ vào ván đang dở (X12) — người dùng không cần
  biết hai đường này khác nhau.

Ván chỉ trở lại `IN_PROGRESS` khi bên đã mất kết nối nối lại được. Trong lúc
`PAUSED`, đối thủ gửi `MOVE` sẽ nhận `ERROR 4001` chứ không phải bị bỏ lơ — client
phải chờ và thử lại, đừng coi đó là mất ván.
...
| 0x22 | S→R PING | rỗng |
| 0x23 | S→R MATERIAL | JSON `{fen, side}` — **RVP v1.1**, hỏi bên `side` có đủ quân chiếu hết không (X29) |
...
`status` ∈ `ongoing | check | checkmate | stalemate | draw_fifty | draw_threefold | draw_insufficient`.

Trả lời `MATERIAL` dùng lại `RULES_OK` với payload `{sufficient: bool}`.

> **Hạn chế đã biết:** `draw_threefold` chỉ phát hiện được khi request mang theo lịch sử ván.
> Cả hai bản hiện thực (chess.js nạp từ FEN, và bản Java đối chứng) đều nhận một thế cờ rời rạc,
> nên lặp 3 lần **không** phát hiện được từ một FEN đơn lẻ. Ghi vào Limitations của báo cáo.
...
| --- | --- | --- |
| 1.2 | **12/10/2026** | Thêm `LOBBY_REQ` (0x10) / `LOBBY_RESULT` (0x8B) để khán giả lấy danh sách ván đang chạy và bảng xếp hạng trong một vòng gọi. `PEER_STATUS` nhận thêm giá trị `searching` cho trường hợp chờ ghép quá lâu (X23). RVP lên v1.1 với `MATERIAL` (0x23) — hết giờ mà đối thủ không đủ quân chiếu hết thì theo luật FIDE là **hoà**, và server phải hỏi được điều đó (X29). Ba thay đổi đều **thêm mới**, không sửa message cũ: client v1.1 vẫn chạy bình thường, chỉ là không dùng được khán đài. |
